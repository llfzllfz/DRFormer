...
class train():
    ...
    def train_one_loop(self):
        ...
        for tmp in tqdm(self.train_dataloader):
            ...
            if self.config.multi_gpu == 0 or (torch.distributed.get_rank() == 0 and self.config.multi_gpu > 0):
...
if __name__ == '__main__':
    ...
    if config.multi_gpu > 0:
        ...
        dist.barrier()
        print('finish init')
    # if config.dataset == 'RSS':
    #     from processing_data.RSS_dataloader import RSS_dataloader
    #     train_dataloader = RSS_dataloader(os.path.join(config.data_path, config.dataset, config.filename), config.command)
    #     test_dataloader = RSS_dataloader(os.path.join(config.data_path, config.dataset, config.filename), config.command)
