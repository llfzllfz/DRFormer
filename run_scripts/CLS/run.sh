...
python train.py --gpu $gpu --k_fold_data_path "origin_data/${dataset}" --dataset "CLS" --seed 42 --multi_gpu 0 \
                --metric 'MCC' \
                $model --batch_size=32 --save_path "${second_path}/${model}.pt" \
                --log_path "${log_paths}/${model}_Predict.log" --epochs 200 --early_stop 200 \
                --dataloader 60 \
                --text_vision \
                --direction --cross \
                --distribution 0 \
