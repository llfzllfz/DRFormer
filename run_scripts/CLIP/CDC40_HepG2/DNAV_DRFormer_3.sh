...
python train.py --filename $filename --gpu $gpu --kfold_k $kfold_k --k_fold_data_path '/data1/llfz/Cycleformer/data/clip' \
                --metric 'AUROC' \
                $model --batch_size 32 --save_path "${second_path}/DRFormer_3_${kfold_k}.pt" \
                --log_path "${log_paths}/DRFormer_3_${kfold_k}.log" --epochs 10 --early_stop 3 \
                --dataloader 60 \
                --text_vision \
                --direction --cross \
                --cross_attention_num_layers 3 \
                --distribution 0 \
                --lr 3e-5 \
                --Vision_lr 1e-4 \
